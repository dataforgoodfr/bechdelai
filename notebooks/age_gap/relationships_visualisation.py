...
def main():
    ...
    for i,row in scores.sort_values('score',ascending=False).iterrows():
        ...
        col1.image('https://image.tmdb.org/t/p/original'+img_star2["profiles"][0]["file_path"],width=200)

        img_star1 = tmdb.get_person_image_from_id(row.star_id1)
        col2.image('https://image.tmdb.org/t/p/original'+img_star1["profiles"][0]["file_path"],width=200)
